...
<head>
	<title>修改</title>
	<#if isRT == 1>
	<script type="text/javascript">window.UEDITOR_HOME_URL = "<@jspEl 'base'/>/ueditor/";</script>
	<script src="<@jspEl 'base'/>/ueditor/ueditor.config.js" type="text/javascript" ></script>
	<script src="<@jspEl 'base'/>/ueditor/ueditor.all.min.js" type="text/javascript" ></script>
	<script src="<@jspEl 'base'/>/ueditor/ueditor.parse.js" type="text/javascript" ></script>
	</#if>
</head>
<body>
	<div class="path">
		<a href="<@jspEl 'base'/>/${classNameLowerCase}<#if isFK == 1><#noparse><#if (</#noparse>${classNameFirstLower}<#noparse>.foreignKey)??>?foreignKey=${</#noparse>${classNameFirstLower}<#noparse>.foreignKey}</#if></#noparse></#if>">返回</a> &raquo; 编辑
	</div>
	<#list table.columns as column>
	<#if column.pk>
    <form action="<@jspEl 'base'/>/${classNameLowerCase}<#noparse><#if (</#noparse>${classNameFirstLower}.${column.columnNameFirstLower}<#noparse>)??>/</#noparse><@jspEl classNameFirstLower+"."+column.columnNameFirstLower/><#noparse></#if></#noparse>" name="${classNameFirstLower}Form" id="${classNameFirstLower}Form" method="post" >
    </#if>
	</#list>
		<#if isFK == 1>
		...
		<table class="input">
			...
			<#list table.columns as column>
				...
				<tr>
					...
					<#noparse><@parameter parentId=</#noparse>${(column.remarks)?substring((column.remarks)?index_of("#EPS")+5,(column.remarks)?index_of("）"))}>
					...
					<#noparse><@parameter parentId=</#noparse>${(column.remarks)?substring((column.remarks)?index_of("#EPC")+5,(column.remarks)?index_of("）"))}>
					...
					<#noparse><@parameter parentId=</#noparse>${(column.remarks)?substring((column.remarks)?index_of("#EPR")+5,(column.remarks)?index_of("）"))}>
					<#noparse><#list  parameterList as para></#noparse>
					...
					<select id="${column.columnNameLower}" name="${column.columnNameLower}" class="">
						...
						<option value="${para?substring(0,para?index_of("-"))}" <#noparse><#if (</#noparse>${classNameFirstLower}.${column.columnNameFirstLower}<#noparse>)??><#if (</#noparse>${classNameFirstLower}.${column.columnNameFirstLower}==<#if column.isStringColumn>"</#if>${para?substring(0,para?index_of("-"))}<#if column.isStringColumn>"</#if><#noparse>)>selected</#if></#if></#noparse>>${para?substring(para?index_of("-")+1)}</option>
						</#list>
					</select>
					...
					<#list listPara?split("、") as para>
					<label class="checkbox-inline"><input type="checkbox" name="${column.columnNameLower}" value="${para?substring(0,para?index_of("-"))}" <#noparse><#if (</#noparse>${classNameFirstLower}.${column.columnNameFirstLower}<#noparse>)??><#if (</#noparse>${classNameFirstLower}.${column.columnNameFirstLower})?index_of("${para?substring(0,para?index_of("-"))}") &gt; -1 >checked<#noparse></#if></#if></#noparse>>${para?substring(para?index_of("-")+1)}</label>
					</#list>
					...
					<#list listPara?split("、") as para>
					<label class="radio-inline"><input type="radio" name="${column.columnNameLower}" value="${para?substring(0,para?index_of("-"))}" <#noparse><#if (</#noparse>${classNameFirstLower}.${column.columnNameFirstLower}<#noparse>)??><#if </#noparse>${classNameFirstLower}.${column.columnNameFirstLower}==<#if column.isStringColumn>"</#if>${para?substring(0,para?index_of("-"))}<#if column.isStringColumn>"</#if>>checked<#noparse></#if></#if></#noparse>>${para?substring(para?index_of("-")+1)}</label>
					</#list>
			...
			<tfoot>
				<tr>
					<td colspan="2">
					<input type="button" value="返回列表" onclick='window.location="<@jspEl 'base'/>/${classNameLowerCase}<#if isFK == 1><#noparse><#if (</#noparse>${classNameFirstLower}<#noparse>.foreignKey)??>?foreignKey=${</#noparse>${classNameFirstLower}<#noparse>.foreignKey}</#if></#noparse></#if>";' class="return-button"/>
