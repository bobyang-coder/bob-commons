	...
	<form action="${ctx}/admin/code" method="get" name="listForm" id="listForm">
		...
		<div class="body-box">
			...
			<table id="listTable" class="list">
				...
				<td>
					<a href="${ctx}/admin/code/<#if (table.tableName)??>${table.tableName}</#if>?url=<#if (dbConfig.url)??>${dbConfig.url}</#if>&driver=<#if (dbConfig.driver)??>${dbConfig.driver}</#if>&username=<#if (dbConfig.username)??>${dbConfig.username}</#if>&password=<#if (dbConfig.password)??>${dbConfig.password}</#if>&schema=<#if (dbConfig.schema)??>${dbConfig.schema}</#if>">修改</a> 
				</td>
				</tr>
				</#list>
			</tbody>
